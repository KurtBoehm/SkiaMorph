...
// https://github.com/google/skia/blob/chrome/m147/modules/svg/src/SkSVGDOM.cpp,
// which is licensed under the license found in LicenseSkia within the project root
// and originally carried the following header:
//
...
bool SetObjectBoundingBoxUnitsAttribute(const sk_sp<SkSVGNode>& node, SkSVGAttribute attr,
                                        const char* stringValue) {
...
// Traverse the XML DOM and collect the concatenated contents of all <style> elements.
SkString collect_style_text(const SkDOM& xmlDom, const SkDOM::Node* node) {
  SkString collected;

  const SkDOM::Type nodeType = xmlDom.getType(node);
  if (nodeType == SkDOM::kElement_Type) {
    const char* name = xmlDom.getName(node);
    if (name && strcmp(name, "style") == 0) {
      // Collect all text children of this <style> node.
      for (auto* child = xmlDom.getFirstChild(node, nullptr); child;
           child = xmlDom.getNextSibling(child)) {
        if (xmlDom.getType(child) == SkDOM::kText_Type) {
          const char* txt = xmlDom.getName(child);
          if (txt && txt[0]) {
    ...
    // Recurse into children (so we also see <style> inside <defs>, etc.).
    for (auto* child = xmlDom.getFirstChild(node, nullptr); child;
         child = xmlDom.getNextSibling(child)) {
      collected.append(collect_style_text(xmlDom, child));
    }
  }
...
bool set_string_attribute(const sk_sp<SkSVGNode>& node, const char* name, const char* value) {
  ...
  const int attrIndex =
    SkStrSearch(&gAttributeParseInfo[0].fKey, SkTo<int>(std::size(gAttributeParseInfo)), name,
...
void parse_node_attributes(const SkDOM& xmlDom, const SkDOM::Node* xmlNode,
  ...
  {
    // Try to find the class of the item
    ...
    if (class_name) {
      const std::string_view class_name_sv{class_name};
      const auto* rulep = stylesheet.match(class_name_sv);
      if (rulep) {
        const auto& rule = *rulep;
        for (const auto& decl : rule.decls) {
          set_string_attribute(svgNode, decl.property.c_str(), decl.value.c_str());
        }
      }
    }
  }

  {
    ...
    while ((name = attrIter.next(&value))) {
      ...
      if (!strcmp(name, "id")) {
        const SkString value_sk(value);
        mapper->set(value_sk, svgNode);
        svgNode->setId(value_sk);
        continue;
      }
      if (!strcmp(name, "class")) {
        continue;
...
sk_sp<SkSVGNode> construct_svg_node(const SkDOM& dom, const ConstructionContext& ctx,
                                    const SkDOM::Node* xmlNode, const css::Stylesheet& stylesheet) {
  ...
  auto make_node = [](const ConstructionContext& ctx, const char* elem) -> sk_sp<SkSVGNode> {
    ...
    const int tagIndex = SkStrSearch(&gTagFactories[0].fKey, SkTo<int>(std::size(gTagFactories)),
  ...
  parse_node_attributes(dom, xmlNode, node, ctx.fIDMapper, stylesheet);

  ConstructionContext localCtx(ctx, node);
  for (auto* child = dom.getFirstChild(xmlNode, nullptr); child;
       child = dom.getNextSibling(child)) {
    sk_sp<SkSVGNode> childNode = construct_svg_node(dom, localCtx, child, stylesheet);
    if (childNode) {
...
sk::SVGDOM::Builder& sk::SVGDOM::Builder::setFontManager(sk_sp<SkFontMgr> fmgr) {
  fFontMgr = std::move(fmgr);
  return *this;
}

sk::SVGDOM::Builder&
sk::SVGDOM::Builder::setResourceProvider(sk_sp<skresources::ResourceProvider> rp) {
  fResourceProvider = std::move(rp);
  return *this;
}

sk::SVGDOM::Builder& sk::SVGDOM::Builder::setTextShapingFactory(sk_sp<SkShapers::Factory> f) {
  fTextShapingFactory = f;
  return *this;
}

sk_sp<sk::SVGDOM> sk::SVGDOM::Builder::make(SkStream& str) const {
  ...
  const auto style = collect_style_text(xmlDom, xmlDom.getRootNode());
  const auto stylesheet = css::parse(std::string_view{style.data(), style.size()});

  ConstructionContext ctx(&mapper);

  auto root = construct_svg_node(xmlDom, ctx, xmlDom.getRootNode(), stylesheet);
  ...
  return sk_sp<sk::SVGDOM>(new sk::SVGDOM(sk_sp<SkSVGSVG>(static_cast<SkSVGSVG*>(root.release())),
                                          std::move(fFontMgr), std::move(resource_provider),
                                          std::move(mapper), std::move(factory)));
}

sk::SVGDOM::SVGDOM(sk_sp<SkSVGSVG> root, sk_sp<SkFontMgr> fmgr,
                   sk_sp<skresources::ResourceProvider> rp, SkSVGIDMapper&& mapper,
                   sk_sp<SkShapers::Factory> fact)
    : fRoot(std::move(root)), fFontMgr(std::move(fmgr)), fTextShapingFactory(std::move(fact)),
      fResourceProvider(std::move(rp)), fIDMapper(std::move(mapper)),
      fContainerSize(fRoot->intrinsicSize(SkSVGLengthContext(SkSize::Make(0, 0)))) {
  SkASSERT(fResourceProvider);
  SkASSERT(fTextShapingFactory);
}

sk_sp<sk::SVGDOM> sk::SVGDOM::MakeFromStream(SkStream& str) {
  return Builder().make(str);
}

void sk::SVGDOM::render(SkCanvas* canvas) const {
  TRACE_EVENT0("skia", TRACE_FUNC);
  if (fRoot) {
    ...
    fRoot->render(SkSVGRenderContext(canvas, fFontMgr, fResourceProvider, fIDMapper, lctx, pctx,
                                     {nullptr, nullptr}, fTextShapingFactory));
  }
}

void sk::SVGDOM::renderNode(SkCanvas* canvas, SkSVGPresentationContext& pctx,
                            const char* id) const {
  TRACE_EVENT0("skia", TRACE_FUNC);

  if (fRoot) {
    SkSVGLengthContext lctx(fContainerSize);
    fRoot->renderNode(SkSVGRenderContext(canvas, fFontMgr, fResourceProvider, fIDMapper, lctx, pctx,
                                         {nullptr, nullptr}, fTextShapingFactory),
                      SkSVGIRI(SkSVGIRI::Type::kLocal, SkSVGStringType(id)));
  }
}

const SkSize& sk::SVGDOM::containerSize() const {
  return fContainerSize;
}

void sk::SVGDOM::setContainerSize(const SkSize& containerSize) {
  // TODO: inval
  fContainerSize = containerSize;
}

sk_sp<SkSVGNode>* sk::SVGDOM::findNodeById(const char* id) {
  SkString idStr(id);
